...
export class LinuxArm64InterruptorAgent extends InterruptorAgent{
    ...
    traceSyscall( pContext:any, pHookCfg:any = null){
        ...
        var inst = "SVC"; //Instruction.parse(pContext.pc).toString();
        ...
        switch(sys[1]){
            ...
            case 'prctl':
                ...
                //break
            case 'fstat':
                s = `${sys[1]} (${pContext.dxcFD[pContext.x0.toInt32()+""]})`; // pContext.dxcFD[pContext.x0.toInt32()+""];
            ...
            default:
                ...
                sys[3].map((vVal,vOff) => {
                    if(typeof vVal === "string"){
                        p += `${vVal} = ${pContext["x"+vOff]} , `;
                    }else{
                        p += `${vVal.n} = `;
                        if(vVal.t == T.CHAR_BUFFER){
                            p += pContext["x"+vOff].readUtf8String();
                        }else{
                            p += pContext["x"+vOff];
                        }
                    }
        ...
        if(this.output.flavor == InterruptorAgent.FLAVOR_DXC){
            console.log( ' ['+loc+']   \x1b[35;01m' + inst + ' :: '+pContext.x8+' \x1b[0m '+s+'  [lr='+pContext.lr+']');
        }
